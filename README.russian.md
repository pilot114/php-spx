...
* [поддержка множества метрик](#available-metrics): в данный момент их 21 (различные временные метрики, память, подключаемые файлы, используемые объекты, I/O...).
...
Предполагая среду разработки [описанную здесь](#private-environment) ваше приложение доступно по адресу `http://localhost`.

Просто откройте в своем браузере следующий URL: `http://localhost/?SPX_KEY=dev&SPX_UI_URI=/` чтобы получить доступ к web UI [панели управления](#control-panel).
...
Нажмите на отчет в списке и наслаждайтесь [экраном анализа](#analysis-screen).
